...
def chatbot():
    ...
    try:
        ...
        knowledge_base = {
            ...
            'lumpy_skin_disease': {
                'keywords': ['lumpy', 'lumpy skin', 'lumpy skin disease', 'lsd', 'nodules', 'skin nodules', 'lump', 'lumps'],
            ...
            'foot_mouth_disease': {
                'keywords': ['foot', 'mouth', 'foot and mouth', 'fmd', 'blisters', 'lameness', 'hoof'],
            ...
            'mastitis': {
                'keywords': ['mastitis', 'udder', 'milk', 'udder infection', 'teat'],
            ...
            'bovine_respiratory_disease': {
                'keywords': ['respiratory', 'breathing', 'cough', 'pneumonia', 'lung', 'brd', 'shipping fever'],
            ...
            'bovine_viral_diarrhea': {
                'keywords': ['bvd', 'viral diarrhea', 'bovine viral diarrhea', 'diarrhea', 'scours'],
            ...
            'nutrition': {
                'keywords': ['feed', 'nutrition', 'food', 'diet', 'grass', 'hay', 'silage', 'grain'],
            ...
            'reproduction': {
                'keywords': ['pregnant', 'pregnancy', 'calving', 'birth', 'breeding', 'heat', 'estrus'],
            ...
            'housing': {
                'keywords': ['housing', 'barn', 'shelter', 'bedding', 'stall', 'pen'],
            ...
            'emergency': {
                'keywords': ['emergency', 'urgent', 'critical', 'help', 'emergency', 'dying', 'dead'],
            ...
            # General questions
            'symptoms': {
                'keywords': ['symptom', 'sign', 'sick', 'ill', 'disease sign'],
            ...
            'veterinarian': {
                'keywords': ['vet', 'veterinarian', 'doctor', 'expert', 'professional'],
            ...
            'milk': {
                'keywords': ['milk', 'milking', 'udder', 'mastitis', 'production'],
            ...
            'calf': {
                'keywords': ['calf', 'calves', 'baby', 'young', 'newborn'],
        ...
        def find_best_response(message):
            ...
            for category, info in knowledge_base.items():
                ...
                keyword_matches = sum(1 for keyword in info['keywords'] if keyword in message)
                if keyword_matches > 0:
                    score = keyword_matches * 2  # Weight keyword matches heavily
            ...
            return best_match, matched_category
        ...
        def generate_response(info, category, message):
            ...
            if any(word in message for word in ['symptom', 'sign', 'show', 'look like']):
                if 'symptoms' in info:
                    response_parts.append(f"**Symptoms of {category.replace('_', ' ').title()}:** {info['symptoms']}")

            elif any(word in message for word in ['cause', 'why', 'how get', 'spread']):
                if 'causes' in info:
                    response_parts.append(f"**Causes of {category.replace('_', ' ').title()}:** {info['causes']}")

            elif any(word in message for word in ['treat', 'cure', 'medicine', 'heal']):
                if 'treatment' in info:
                    response_parts.append(f"**Treatment for {category.replace('_', ' ').title()}:** {info['treatment']}")

            elif any(word in message for word in ['prevent', 'avoid', 'stop', 'protection']):
                if 'prevention' in info:
                    response_parts.append(f"**Prevention of {category.replace('_', ' ').title()}:** {info['prevention']}")

            elif any(word in message for word in ['severe', 'bad', 'danger', 'mortality']):
                if 'severity' in info:
                    response_parts.append(f"**Severity of {category.replace('_', ' ').title()}:** {info['severity']}")

            else:
                # General response
                if 'response' in info:
                    response_parts.append(info['response'])
                elif 'symptoms' in info:
                    response_parts.append(f"**About {category.replace('_', ' ').title()}:** {info['symptoms'][:200]}...")

            # Add general advice if not specific
            if len(response_parts) == 0:
                response_parts.append("I can provide information about various cow diseases, health management, nutrition, and care. Please ask about specific symptoms, treatments, or prevention measures.")

            return " ".join(response_parts)

        # Find best response
        best_match, category = find_best_response(user_message)
